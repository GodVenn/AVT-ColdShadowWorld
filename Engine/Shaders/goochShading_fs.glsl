...
in float exDiffuseIntensity; // Difusse intensity 
...
void main(void)
{
  ...
  vec3 diffuse = mix(kcool, kwarn, exDiffuseIntensity);
  ...
  vec4 shadowDiffuseColor = vec4(diffuse * 0.6f, 1.0);
  //shadowDiffuseColor.a = 0.6;
  // Interpolate between the shadow and the final gooch component
